...
// Custom basename function to avoid modifying input strings
...
void *LoadLibrary(const char *name) {
    ...
    for (uint32_t i = 0; i < all_image_infos->infoArrayCount; ++i) {
        if(strcmp(all_image_info_array[i].imageFilePath, name) == 0) {
            // printf("Matched library: %s\n", all_image_info_array[i].imageFilePath);
            return (void*)all_image_info_array[i].imageLoadAddress;
        }
    }

    // 如果完整路径未匹配，尝试匹配 basename
    for (uint32_t i = 0; i < all_image_infos->infoArrayCount; ++i) {
        const char* image_basename = my_basename(all_image_info_array[i].imageFilePath);
        const char* name_basename = my_basename(name);
        if (strcmp(image_basename, name_basename) == 0) {
            printf("Matched library by basename: %s\n", image_basename);
            printf("Library load address: %p\n", (void*)all_image_info_array[i].imageLoadAddress);
            return (void*)all_image_info_array[i].imageLoadAddress;
        }
    }
...
void *GetAddressFromFileOffset(void *base_address, uint64_t target_file_offset) {
    struct mach_header_64 *mach_header = (struct mach_header_64 *)base_address;
    ...
    uint64_t slide = (uint64_t)base_address - text_cmd->vmaddr;
    ...
    for (uint32_t i = 0; i < mach_header->ncmds; ++i) {
        ...
        if (load_cmd->cmd == LC_SEGMENT_64) {
            ...
            if (target_file_offset >= seg->fileoff && target_file_offset < seg->fileoff + seg->filesize) {
                uint64_t offset_in_segment = target_file_offset - seg->fileoff;
                return (void*)(seg->vmaddr + slide + offset_in_segment);
            }
        }
        load_cmd_addr += load_cmd->cmdsize;
    }
